...
export const COLUMNS: Column<SupplyMaterialRow>[] = [
  ...
  },
  {
    ...
    changeValue: (row) => row.days_left,
    render: (row) =>
      row.days_left === null ? (
        // Прочерк, а не ноль: остатка в отчёте нет вовсе либо расхода
        // за период не было. Ноль означал бы «кончился» — другое
        // утверждение об учёте.
        <span className="text-muted-foreground">—</span>
      ) : (
        <span
          className={
            // Цвет только там, где есть о чём предупредить, и всегда
            // вместе с числом: `DESIGN.md` §1.
            row.days_left_level === "critical"
              ? "text-destructive"
              : row.days_left_level === "low"
                ? "text-warning"
                : undefined
          }
        >
          {withPlural(row.days_left, "день", "дня", "дней")}
        </span>
      ),
    explain: (
      <Explain>
        <b>Свободный остаток ÷ расход в день.</b> Расход берётся из отгрузок
        через техкарты, а не из приёмок: приёмка говорит, сколько пришло,
        а вопрос «пора ли закупать» — про то, сколько уходит. Тот же расчёт,
        что в раскрытии строки, и числа обязаны совпасть. Прочерк там, где
        остатка нет в отчёте либо материал за период не расходовался.
      </Explain>
    ),
  },
]
