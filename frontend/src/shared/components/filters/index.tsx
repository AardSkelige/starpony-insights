...
function PickerField({
  ...
  return (
      ...
      >
        ...
        {/* Ширина — по содержимому, а не по полю. Список из реестра берёт
            `w-(--anchor-width)`, то есть ширину триггера (176 точек),
            и режет всё, что длиннее: «ООО "ХИМПИТЕРТОРГ ГРУПП"» и
            «ООО "ТИПОГРАФИЯ "ПОЛИГРАН"» превращались в обрубки, по которым
            поставщика не отличить. Снизу ограничение остаётся шириной поля —
            иначе короткий список выглядел бы уже, чем то, из чего он вылез;
            сверху — экраном, чтобы длинное имя не уводило список за край. */}
        <SelectContent
