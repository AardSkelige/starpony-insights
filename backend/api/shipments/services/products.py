...
ORDERING = {
    ...
    "share": REVENUE,
    "-share": f"-{REVENUE}",
    # По цене из карточки. Нули — наверх: это и есть «цена не задана», ради
    # чего главная сюда и присылает. А `null` (товара нет на складе) — вниз:
    # там о цене вопроса не стоит.
    "card_price": F("product__stock__sale_price_kopecks").asc(nulls_last=True),
    "-card_price": F("product__stock__sale_price_kopecks").desc(nulls_last=True),
}
...
def grouped(filters: Filters) -> QuerySet:
    ...
    return (
        ...
        .values(
            ...
            "product__code",
            "product__uom__name",
            # Цена продажи из карточки МойСклада. Приходит вместе с товаром,
            # а не отдельным запросом: строк на странице десять, товаров
            # в выборке — шестьдесят шесть.
            "product__stock__sale_price_kopecks",
        )
        .annotate(**_sums())
...
def row_of(item: dict, total_revenue: int) -> dict:
    ...
    return {
        ...
        # Цена из карточки товара — рядом с фактической, по которой продавали.
        #
        # **Ноль и `null` здесь означают разное, и путать их нельзя.** Ноль —
        # цена в карточке не задана: товар лежит на складе, а продать его
        # нельзя, и это сигнал главной. `null` — строки остатка нет вовсе,
        # товара на складе не было, и вопроса о его цене не возникает.
        # Свернув одно в другое, список «без цены» наполнился бы позициями,
        # которых просто нет в наличии.
        #
        # Расхождение с фактической ценой — отдельный вопрос, и он виден
        # только когда обе цены стоят рядом: продавали дешевле прайса или
        # дороже, замечают именно так.
        "card_price_kopecks": item["product__stock__sale_price_kopecks"],
