...
def row_of(
    ...
    return {
        ...
        # Имена покупателей — только для поиска, наружу не уходят. Все,
        # а не первая пятёрка из `buyers`: искать «Каприоль» и не найти
        # канал, потому что он шестой по величине, — это поиск, которому
        # перестают верить.
        "agent_names": {shipment.agent.name for shipment in shipments},
...
def _matches(row: dict, term: str) -> bool:
    """Поиск по названию канала **и по покупателю**.

    Строка таблицы — канал, но вопрос к странице часто звучит иначе:
    «через что покупает Каприоль». Ответ — тот же канал, просто найденный
    с другой стороны, и заставлять человека сначала вспоминать название
    канала незачем.

    **Выдача остаётся прежней:** находятся каналы, а не покупатели, — иначе
    строка таблицы означала бы то одно, то другое. Совпадение по покупателю
    только добавляет способ попасть в нужную строку.

    Числа при этом не сужаются: поиск отбирает показанные строки, а доли
    и сводка считаются по всей выборке (`DESIGN.md` §8).
    """
    needle = term.strip().casefold()
    if needle in row["name"].casefold():
        return True
    return any(needle in name.casefold() for name in row["agent_names"])
