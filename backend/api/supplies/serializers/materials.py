...
class SupplyMaterialRowSerializer(serializers.Serializer):
    ...
    # На сколько дней хватит остатка при нынешнем расходе. `null` — остатка
    # в отчёте нет вовсе либо расхода за период не было: ноль здесь означал
    # бы «кончился», а это другое утверждение об учёте.
    #
    # Заведено 04.09: главная присылает сюда по сигналу «сырья кончится
    # за 30 дней», а показать это было нечем — страница открывалась своей
    # сортировкой по сумме закупки, и восемь позиций приходилось искать
    # глазами среди двухсот.
    days_left = serializers.IntegerField(allow_null=True)
    days_left_level = serializers.ChoiceField(choices=["none", "ok", "low", "critical"])
