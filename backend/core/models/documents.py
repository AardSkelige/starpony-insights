...
**Заказы приходят без позиций** — и заказ поставщику, и заказ покупателя.
Это проверено, а не принято из удобства. Позиции у него есть (415 строк в 96 заказах), и `expand` их
...
class Document(MirrorModel):
    ...
    # Комментарий из учёта. Хранится, потому что в нём лежит **причина**:
    # у отгрузки — про накладные расходы («самовывоз», «доставку оплачивал
    # получатель»), а у заказа покупателя — зачем товар ушёл вообще.
    #
    # Из 53 отгрузок с нулевой ценой заказ есть у всех 53, и комментарий —
    # тоже у всех: «на призы на ЧР-2026 по конкуру», «подарок потенциальному
    # оптовику», «замена взамен вытекшей бутылки». Это и есть ответ на «куда
    # ушли 532 штуки», написанный человеком, — лучше любого справочника
    # категорий, который всё равно никто не будет заполнять.
    ...
    # Заказ, из которого выросла отгрузка. Заполнен только у отгрузок,
    # и нужен ровно за одним — комментарием: в отгрузке пишут про накладные
    # расходы, а причину отгрузки пишут в заказе.
