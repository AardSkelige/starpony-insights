...
class Stock(DomainModel):
    ...
    # Цена продажи из карточки товара. Приходит тем же отчётом, что и остаток,
    # — отдельных запросов не стоит вовсе.
    #
    # Ноль здесь означает «цена не задана»: в отчёте поле обязательное, и товар
    # без цены приходит с `salePrice: 0.0`. Отсюда сигнал главной «товары
    # без цены продажи» — продать по нулю нельзя, значит позиция выпала
    # из продажи молча.
    sale_price_kopecks = models.DecimalField(
        "Цена продажи, копейки", max_digits=18, decimal_places=6, default=0
    )
