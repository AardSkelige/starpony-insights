...
def sync_commission_reports(client: MoySkladClient, run: SyncRun) -> EntityOutcome:
    """Полученные отчёты комиссионера — вместе с позициями.
    ...
    **Позиции нужны, хотя «Срокам оплаты» хватало шапки.** Отгрузка
    на реализацию — ещё не продажа: выручка возникает в момент отчёта.
    Разница между отгруженным и проданным по договорам комиссии — 281 126 ₽
    на 02.09, и без позиций её нельзя разложить по товарам, а значит нельзя
    и пометить строку словами «отгружено, ещё не продано». Отчётов в учёте 12,
    и `expand` их не стоит ни одного лишнего запроса.
    """
    ...
    _warn_about_commission_returns(client)
    return outcome


def _warn_about_commission_returns(client: MoySkladClient) -> None:
    """Возврат на склад комиссионера — вторая коллекция позиций отчёта.

    В аккаунте их сегодня **ноль во всех двенадцати отчётах** (проверено),
    поэтому кода для них нет: неиспользуемая ветка через месяц будет принята
    за источник правды и посчитана. Но появись первый возврат — «лежит
    у комиссионера» окажется завышено ровно на него, и молча.

    Считается по `meta.size`, который приходит в том же ответе: лишних
    запросов это не стоит.
    """
    returned = 0
    try:
        for row in client.iterate("/entity/commissionreportin"):
            returned += (
                (row.get("returnToCommissionerPositions") or {})
                .get("meta", {})
                .get("size")
                or 0
            )
    except Exception:
        # Предупреждение не должно ронять прогон: оно про будущее,
        # а документы уже сохранены.
        return

    if returned:
        logger.warning(
            "Отчёты комиссионера: %s позиций возврата на склад комиссионера "
            "не учитываются. «Лежит у комиссионера» на «Прибыльности» "
            "завышено — пора завести их в расчёт.",
            returned,
        )
